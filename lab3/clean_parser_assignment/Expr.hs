...
-- Need value pattern match for possible expressions
value :: Expr -> Dictionary.T String Integer -> Integer
value (Num n) _ = n
value (Var x) env = case Dictionary.lookup x env of
        Just y -> y 
        _ -> error (x ++ " undefined var "++ x) 
...
value (Div left right) env = 
        if value right env == 0 
                then error "cant divide by zero" 
                        else value left env `div` value right env
